		...
		local function GetAltverPriority(word)
			...
			Title.Text = "Imitate<font color=\"rgb(196,40,28)\">Altver</font> V25"
			Title.RichText = true
		...
		local SortBtn = CreateToggle("Sort: "..sortMode, UDim2.new(0, 15, 0, 33), function()
			...
			local function CreateSartreCustomizer()
				local SartreFrame = Instance.new("Frame")
				SartreFrame.Name = "SartreCustomizer"
				SartreFrame.Size = UDim2.new(0, 340, 0, 440)
				SartreFrame.Position = UDim2.new(0.5, -170, 0.5, -220)
				SartreFrame.BackgroundColor3 = THEME.Background
				SartreFrame.Visible = false
				SartreFrame.ClipsDescendants = true
				SartreFrame.Parent = ScreenGui

				EnableDragging(SartreFrame)
				Instance.new("UICorner", SartreFrame).CornerRadius = UDim.new(0, 8)

				local stroke = Instance.new("UIStroke", SartreFrame)
				stroke.Color = THEME.Accent
				stroke.Transparency = 0.4
				stroke.Thickness = 2

				local header = Instance.new("TextLabel", SartreFrame)
				header.Text = "Customize Sartre Priority"
				...
				local closeBtn = Instance.new("TextButton", SartreFrame)
				closeBtn.Text = "X"
				closeBtn.Font = Enum.Font.GothamBold
				...
				searchBox.PlaceholderText = "Search endings..."
				searchBox.Font = Enum.Font.Gotham
				searchBox.TextSize = 14
				...
				-- Scroll Frame
				local scroll = Instance.new("ScrollingFrame", SartreFrame)
				...
				-- Add New Section
				local addFrame = Instance.new("Frame", SartreFrame)
				addFrame.Size = UDim2.new(1, -20, 0, 34)
				addFrame.Position = UDim2.new(0, 10, 1, -45)
				addFrame.BackgroundTransparency = 1

				local endingBox = Instance.new("TextBox", addFrame)
				endingBox.Size = UDim2.new(0.42, 0, 1, 0)
				endingBox.PlaceholderText = "Ending (e.g. tion)"
				endingBox.Font = Enum.Font.Gotham
				endingBox.TextSize = 13
				endingBox.BackgroundColor3 = THEME.ItemBG
				Instance.new("UICorner", endingBox).CornerRadius = UDim.new(0, 6)

				local valueBox = Instance.new("TextBox", addFrame)
				valueBox.Size = UDim2.new(0.25, 0, 1, 0)
				valueBox.Position = UDim2.new(0.44, 0, 0, 0)
				valueBox.PlaceholderText = "Priority"
				valueBox.Font = Enum.Font.Gotham
				valueBox.TextSize = 13
				valueBox.BackgroundColor3 = THEME.ItemBG
				Instance.new("UICorner", valueBox).CornerRadius = UDim.new(0, 6)

				local addBtn = Instance.new("TextButton", addFrame)
				addBtn.Text = "Add"
				addBtn.Size = UDim2.new(0.28, 0, 1, 0)
				addBtn.Position = UDim2.new(0.71, 0, 0, 0)
				...
				-- Button in main menu
				local SartreBtn = Instance.new("TextButton", TogglesFrame)
				SartreBtn.Text = "Customize Sartre"
				SartreBtn.Font = Enum.Font.GothamMedium
				SartreBtn.TextSize = 11
				...
				local function RefreshList()
					...
					if query == "search endings..." or query == "" then query = "" end

					local items = {}
					for ending, prio in pairs(PriorityEndings) do
						if query == "" or ending:find(query) then
							table.insert(items, {ending = ending, priority = prio})
						end
					end

					table.sort(items, function(a, b)
						if a.priority == b.priority then return a.ending < b.ending end
					...
					for _, item in ipairs(items) do
						...
						prioBox.TextSize = 14
						prioBox.BackgroundColor3 = Color3.fromRGB(50, 50, 55)
						prioBox.TextColor3 = THEME.Accent
						prioBox.ClearTextOnFocus = false
						Instance.new("UICorner", prioBox).CornerRadius = UDim.new(0, 5)

						local saveBtn = Instance.new("TextButton", row)
						saveBtn.Text = "Save"
						saveBtn.Size = UDim2.new(0, 55, 0, 26)
						saveBtn.Position = UDim2.new(0.68, 0, 0.5, -13)
						...
						delBtn.Position = UDim2.new(0.88, 0, 0.5, -13)
						...
						prioBox.FocusLost:Connect(function(enterPressed)
						...
						saveBtn.MouseButton1Click:Connect(function()
							...
							if num then
								...
								RefreshList()
								forceUpdateList = true
								ShowToast(item.ending .. " priority updated to " .. num, "success")
							end
						end)

						delBtn.MouseButton1Click:Connect(function()
							PriorityEndings[item.ending] = nil
							RefreshList()
							forceUpdateList = true
							ShowToast("Removed ending: " .. item.ending, "warning")
						end)
					end
				...
				addBtn.MouseButton1Click:Connect(function()
					local ending = endingBox.Text:lower():gsub("%s+", "")
					local prio = tonumber(valueBox.Text)

					if #ending >= 2 and prio then
						PriorityEndings[ending] = prio
						endingBox.Text = ""
						valueBox.Text = ""
						RefreshList()
						forceUpdateList = true
						ShowToast("Added: " .. ending .. " = " .. prio, "success")
					else
						ShowToast("Invalid ending or priority number!", "error")
					end
				end)
